...
event_handlers: [256]?EventHandler = blk: {
    var ev_hndls: [256]?EventHandler = .{null} ** 256;
    ev_hndls[0] = handleError;
    ev_hndls[c.XCB_KEY_PRESS] = handleKeyEvent;
    ev_hndls[c.XCB_KEY_RELEASE] = handleKeyEvent;
    ev_hndls[c.XCB_CREATE_NOTIFY] = handleMapRequest;
...
fn handleKeyEvent(self: *Self, ev: *c.xcb_generic_event_t) !void {
    ...
    if (self.key_handlers.get(.{ .keycode = keycode, .state = state, .action = action })) |key_handl| {
        try key_handl();
    }
}

fn handleMapRequest(self: *Self, ev: *c.xcb_generic_event_t) !void {
    const window: c.xcb_window_t = switch (ev.*.response_type) {
        c.XCB_CREATE_NOTIFY => @as(*c.xcb_create_notify_event_t, @ptrCast(ev)).window,
        c.XCB_MAP_REQUEST => @as(*c.xcb_map_request_event_t, @ptrCast(ev)).window,
        else => unreachable,
    };
...
pub fn eventLoop(self: *Self) !void {
    while (@as(?*c.xcb_generic_event_t, @ptrCast(c.xcb_wait_for_event(self.conn)))) |ev| {
        defer c.free(ev);
        if (self.event_handlers[ev.response_type]) |ev_handl| {
            log.debug("handling event: {}", .{ev.response_type});
            try ev_handl(self, ev);
        } else {
    ...
    // set up keybindings
    {
        ...
        const numloc_mod: c.xcb_mod_mask_t = blk: {
            ...
            for (mod_keys[0..@intCast(mod_keys_len)], 0..) |mod_key, i| {
                ...
                break :blk @intCast(switch (i / group_size) {
        ...
        for (presets.default.key_mappings) |key_map| {
            ...
            const action = key_map.action;
            const func = key_map.func;
